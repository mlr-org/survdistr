...
// [[Rcpp::export]]
NumericMatrix c_interp_hazard_mat(
  ...
  NumericMatrix hazard(n_obs, n_eval);
  ...
  for (int i = 0; i < n_obs; i++) {
    ...
    for (int k = 0; k < n_eval; k++) {
      ...
      // ----- t = 0 -----
      // t = 0 => h(0) = 0 if only one anchor at 0, else the value from the first interval (0, times[0])
      if (t == 0.0) {
        // hazard is zero except at anchors >0
        if (m == CONST_SURV) {
          hazard(i, k) = 0.0;
          continue;
        }
        ...
        if (m == CONST_DENS) {
          hazard(i, k) = (S_left - S_right) / delta;
        } else { // CONST_HAZ
          hazard(i, k) = -std::log(S_right > 0.0 ? S_right : eps) / delta;
        }
        continue;
      }
      ...
      if (j >= 0 && times[j] == t) {
        ...
        if (m == CONST_SURV) {
          // h(t) != 0 only at anchors for constant survival interpolation
          hazard(i, k) = S_left == 0.0 ? 0.0 : 1.0 - (S_right / S_left);
        } else if (m == CONST_DENS) {
          // avoid unnessary division by zero
          if (S_right == 0.0) {
            hazard(i, k) = 0.0;
          } else {
            hazard(i, k) = ((S_left - S_right) / delta) / S_right;
          }
        } else { // CONST_HAZ
          // avoid unnessary division by zero
          if (S_left == 0.0) {
            ...
            hazard(i, k) = -std::log(ratio) / delta;
          }
        }
        continue;
      }
      ...
      if (j == B - 1) {
        ...
        if (m == CONST_DENS) {
          ...
          // If S_left == S_right > 0 (last interval is flat) then fB = 0 and t_star = +Inf => h(t) = 0
          hazard(i, k) = (t < t_star) ? fB / (S_right - fB * (t - t_right)) : 0.0;
        } else { // CONST_HAZ
